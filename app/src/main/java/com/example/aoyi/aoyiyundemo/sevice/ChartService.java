package com.example.aoyi.aoyiyundemo.sevice;
...
public class ChartService {
    ...
    public void setXYMultipleSeriesRenderer(double maxX, double maxY,
        ...
        multipleSeriesRenderer.setXLabels(11);
        multipleSeriesRenderer.setYLabels(9);
        multipleSeriesRenderer.setXLabelsAlign(Paint.Align.RIGHT);
        multipleSeriesRenderer.setYLabelsAlign(Paint.Align.RIGHT);
        multipleSeriesRenderer.setAxisTitleTextSize(40);
        ...
        multipleSeriesRenderer.setPointSize(4f);//曲线描点尺寸
        multipleSeriesRenderer.setFitLegend(true);
        multipleSeriesRenderer.setMargins(new int[]{0, 125, 35, 30});//距离上下左右各边的距离上左下右
        multipleSeriesRenderer.setShowGrid(true);
        multipleSeriesRenderer.setZoomEnabled(false, false);//设置渲染器允许放大缩小
        multipleSeriesRenderer.setAxesColor(axeColor);
        multipleSeriesRenderer.setPanEnabled(false);//设置是否允许拖动
        multipleSeriesRenderer.setGridColor(gridColor);
        multipleSeriesRenderer.setBackgroundColor(Color.WHITE);//背景色
        multipleSeriesRenderer.setMarginsColor(Color.blue(1));//边距背景色，默认背景色为黑色，这里修改为白色
